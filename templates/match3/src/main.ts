...
start.onclick=()=>{resetPointer();game.start();canvas.focus({preventScroll:true});};
...
let down=-1,pointer:number|null=null;
function resetPointer(){down=-1;if(pointer!==null&&canvas.hasPointerCapture(pointer))canvas.releasePointerCapture(pointer);pointer=null;}
canvas.onpointerdown=e=>{if(pointer!==null||!e.isPrimary||e.button!==0)return;down=position(e);pointer=e.pointerId;canvas.setPointerCapture(pointer);};
canvas.onpointerup=e=>{if(e.pointerId!==pointer)return;const up=position(e);if(down>=0&&up>=0){if(down!==up&&game.adjacent(down,up))game.swap(down,up);else game.select(up);}resetPointer();};
canvas.onpointercancel=e=>{if(e.pointerId===pointer)resetPointer();};
canvas.onlostpointercapture=e=>{if(e.pointerId===pointer)resetPointer();};
