    ...
    this.time.delayedCall(150, () => {
      for (const t of this.towerTypes) {
        ...
        try {
          ...
          // Keep icons on the version's local resource URL; opaque preview origins
          // cannot safely convert cross-origin texture pixels into a data URL.
          const url = (assetURLs as Record<string, string>)[t.textureKey];
