...
export abstract class BaseTDScene extends Phaser.Scene {
  ...
  private launchUI(): void {
    this.scene.launch('UIScene', {
      ...
      currentWave: 1,
      totalWaves: this.getWaveDefinitions().length,
    });
  }
  ...
  shutdown(): void {
    ...
    // Phaser may already have destroyed its groups before our shutdown hook.
    if (this.towerSlotGroup?.children) this.towerSlotGroup.clear(true, true);
    this.towerGrid.clear();
    // Remove only game-domain subscriptions; retain Phaser's lifecycle hooks.
    for (const name of ['towerTypeSelected', 'towerTypeDeselected', 'spawnEnemy',
      'enemyKilled', 'enemyReachedEnd', 'waveStart', 'waveComplete', 'waveReward',
      'obstacleDestroyed', 'goldChanged', 'livesChanged', 'waveChanged',
      'waveTimerStarted', 'towerTypesReady', 'showCombo', 'showWaveBonus', 'allWavesComplete']) {
